...
1. **Fetch Verified Data**: Downloads actual closing prices from Yahoo Finance for:
   ...
   - Bonds: 10-Year and 5-Year Treasury yields

2. **Extract LLM Data** (parallel): Extracts numerical data from each LLM source into structured format

3. **Compare Against Verified** (with thinking): Compares LLM values against Yahoo Finance data, flags inaccurate sources
...
This ensures that even if all three LLM sources have wrong data, the output uses the correct verified values.
