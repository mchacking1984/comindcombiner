...
export default function Home() {
  ...
  const renderMarkdown = (text) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const isStructural = 
        ...
        trimmed.startsWith('* ') ||
        trimmed.startsWith('- ') ||
        (trimmed.startsWith('**') && trimmed.endsWith('**') && (trimmed.endsWith(':**') || trimmed.length < 40)) ||
      ...
      if (isStructural) {
        ...
        if (lastIndex >= 0) {
          ...
          const lastIsStructuralEnd = lastLine === '---' || 
            (lastLine.startsWith('**') && lastLine.endsWith('**') && (lastLine.endsWith(':**') || lastLine.length < 40));
    ...
    return mergedLines.map((line, i) => {
      ...
      if (trimmed.startsWith('**') && trimmed.endsWith('**') && (trimmed.endsWith(':**') || trimmed.length < 40)) {
  ...
  const consolidate = async () => {
    ...
    setError('');
    setOutput('');
    setStatus('Consolidating with Gemini 2.5 Pro...');
    ...
    try {
      ...
      const marketMoodIndex = content.indexOf('## Market Mood');
      if (marketMoodIndex > 0) {
        content = content.substring(marketMoodIndex);
      }
          ...
          <div className="mt-8 bg-slate-800/50 border border-slate-700 rounded-lg p-6">
            ...
            <div className="mt-6 pt-4 border-t border-slate-700">
              ...
              <p className="text-sm text-slate-400">
                Gemini 2.5 Pro with Google Search grounding for accurate data. 
                For conflicts: if 2 of 3 sources agree, it uses the majority. If all 3 differ, it searches with the specific date to get the correct historical close.
              </p>
            </div>
          </div>
        )}
