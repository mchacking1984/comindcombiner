...
async function fetchYahooChart(symbol, targetDate) {
  try {
    // Start 14 days before target
    const startDate = new Date(targetDate + 'T00:00:00Z');
    startDate.setDate(startDate.getDate() - 14);
    
    // End at 23:59:59 UTC on target date - this ensures we get target date but NOT the day after
    ...
    for (let i = 0; i < chart.timestamp.length; i++) {
      if (quotes.close[i] !== null) {
        ...
        // CRITICAL: Only include data on or before target date
    ...
    // After filtering, the last item is the target date (or last trading day before it)
...
function formatMarketDataForComparison(marketData, targetDate) {
  ...
  output += `Requested Date: ${targetDate}\n\n`;
  ...
  for (const category of categories) {
    ...
    for (const [assetName, data] of Object.entries(marketData)) {
      ...
      const changeStr = data.percentChange !== null 
        ? `${data.percentChange > 0 ? '+' : ''}${data.percentChange.toFixed(2)}% (${direction})` 
        : 'N/A';
      ...
      // Show actual date used (in case market was closed on requested date)
      const dateNote = data.date !== targetDate ? ` (data from ${data.date})` : '';
      
      output += `- ${assetName}: ${priceStr} | Change: ${changeStr} from ${data.previousDate || 'N/A'} to ${data.date} | Sources: ${data.agreeing.join(', ')} ${confidenceEmoji}${dateNote}\n`;
