...
async function fetchMarketData(targetDate) {
  ...
  // targetDate is YYYY-MM-DD format (e.g., "2025-12-30")
  // We want the close ON this date, and the change FROM the previous trading day
  
  // Get data from 14 days before to ensure we have enough history (weekends, holidays)
  const target = new Date(targetDate + 'T12:00:00Z');
  const startDate = new Date(target);
  startDate.setDate(startDate.getDate() - 14);
  
  // End date should be the target date (not beyond)
  const period1 = Math.floor(startDate.getTime() / 1000);
  const period2 = Math.floor(target.getTime() / 1000) + 43200; // target date + 12 hours buffer
  ...
  for (const [symbol, name] of Object.entries(MARKET_SYMBOLS)) {
    try {
      ...
      // Find the exact target date or the last trading day before/on target
      let targetIndex = -1;
      ...
      if (targetIndex === -1) {
        errors.push(`${name}: No data for ${targetDate}`);
        continue;
      }
      ...
      results[name] = {
        ...
        previousClose: prevData?.close || null,
    ...
    for (const asset of assets) {
      ...
      if (data) {
        const changeStr = data.percentChange !== null 
          ? `${data.percentChange > 0 ? '+' : ''}${data.percentChange}% (${data.direction})` 
          : 'N/A';
        const priceStr = data.close.toLocaleString('en-US', {
          minimumFractionDigits: 2,
          maximumFractionDigits: asset.includes('Yield') ? 3 : 2,
        });
        output += `- ${asset}: ${priceStr} on ${data.date} | Change: ${changeStr} (from ${data.previousDate || 'N/A'})\n`;
      } else {
